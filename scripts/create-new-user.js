...
async function createNewUser() {
  ...
  try {
    ...
    if (updateAuthError) {
      console.error('Error updating email in auth system:', updateAuthError.message);
    } else {
    ...
    if (newUserError) {
      console.error('Error checking for new user in public.users:', newUserError.message);
    ...
    if (signInError) {
      console.error('Error signing in with original email:', signInError.message);
      
      // Try with the new email
      console.log('\n7. Attempting to sign in with the new email...');
      
      const { data: signInNewData, error: signInNewError } = await supabase.auth.signInWithPassword({
        email: newEmail,
        password
      });
      
      if (signInNewError) {
        console.error('Error signing in with new email:', signInNewError.message);
      } else {
        ...
        console.log('\nNOTE: The user can now sign in with the email:', newEmail);
        console.log('You may want to update the application to handle both emails or update the user\'s email in the auth system.');
      }
    } else {
