...
async function runMigration() {
  ...
  try {
    ...
    const execSqlPath = path.join(__dirname, '../supabase/migrations/20240724_add_exec_sql_function.sql');
    ...
    try {
      ...
      if (execSqlError) {
        ...
        const { error: createError } = await supabase.from('_exec_sql_create').select('*');
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/20240724_fix_auth_system.sql');
    ...
    try {
      ...
      if (error) {
        console.error('Error executing migration with exec_sql:', error.message);
      } else {
        console.log('Migration executed successfully with exec_sql');
      }
    } catch (err) {
      console.error('Exception executing migration with exec_sql:', err.message);
    }
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (stmtError) {
          console.log(`Statement ${i+1}/${statements.length} failed with exec_sql, continuing...`);
        } else {
          console.log(`Statement ${i+1}/${statements.length} executed successfully`);
        }
      } catch (stmtExecError) {
        console.log(`Statement ${i+1}/${statements.length} execution exception, continuing...`);
      }
    }
    ...
    try {
      const { data: diagData, error: diagError } = await supabase.rpc('diagnose_auth_system');
      ...
      if (diagError) {
        ...
        try {
          const { data: oldDiagData, error: oldDiagError } = await supabase.rpc('diagnose_auth_issues');
          
          if (oldDiagError) {
            console.error('Error running diagnose_auth_issues:', oldDiagError.message);
          } else {
            console.log('Auth system diagnostics (old):', JSON.stringify(oldDiagData, null, 2));
          }
        } catch (oldDiagErr) {
          console.error('Exception running diagnose_auth_issues:', oldDiagErr.message);
        }
      } else {
        console.log('Auth system diagnostics:', JSON.stringify(diagData, null, 2));
      }
    } catch (diagErr) {
    ...
    try {
      const { data: testData, error: testError } = await supabase.rpc('create_auth_user_robust', {
      ...
      if (testError) {
        console.error('Error testing user creation with create_auth_user_robust:', testError.message);
        ...
        try {
          ...
          if (adminError) {
            console.error('Error testing user creation with admin API:', adminError.message);
          } else {
            console.log('Test user creation with admin API succeeded:', adminData);
            
            // Clean up test user
            if (adminData && adminData.user) {
              console.log('Cleaning up test user...');
              try {
                const { error: deleteError } = await supabase.auth.admin.deleteUser(adminData.user.id);
                if (deleteError) {
          ...
          console.error('Exception testing user creation with admin API:', adminErr.message);
        }
      } else {
