...
async function testBypassAuthWithSQL() {
  ...
  try {
    ...
    try {
      ...
      if (manualUserError) {
        if (manualUserError.message.includes("does not exist")) {
          ...
          console.log(
            "manual_user_registry table does not exist, creating it...",
          );
          ...
          console.error(
            "Error checking manual_user_registry:",
            manualUserError.message,
          );
        }
      } else if (manualUserData && manualUserData.length > 0) {
        console.log(
          "User already exists in manual_user_registry:",
          manualUserData[0],
        );
      } else {
        ...
        if (insertError) {
          console.error(
            "Error adding user to manual_user_registry:",
            insertError.message,
          );
        } else {
    ...
    console.log(
      "After running the SQL above in the Supabase SQL Editor, you can test the bypass auth with:",
    );
    console.log("---");
    console.log(
      `SELECT * FROM public.bypass_auth_for_user('${email}', '${password}');`,
    );
    console.log("---");
    console.log("And then generate a session with:");
    console.log("---");
    console.log(
      `SELECT * FROM public.generate_session_for_bypassed_user('${tableData.id}');`,
    );
    console.log("---");
...
// Run the function
testBypassAuthWithSQL().catch((err) => {
