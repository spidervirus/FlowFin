...
async function recreateAuthUser() {
  ...
  try {
    ...
    if (createError) {
      ...
      const { data: createDataNoId, error: createErrorNoId } =
        await supabase.auth.admin.createUser({
          email,
          password,
          email_confirm: true,
      ...
      if (createErrorNoId) {
        console.error(
          "Error creating user without ID:",
          createErrorNoId.message,
        );
      } else {
        console.log(
          "User created successfully without specific ID:",
          createDataNoId,
        );
