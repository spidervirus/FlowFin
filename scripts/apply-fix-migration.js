...
async function applyFixMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      "../supabase/migrations/20240725_fix_auth_user.sql",
    );
    ...
    try {
      ...
      if (error) {
        ...
        if (createFuncError) {
          console.error(
            "Error creating temporary function:",
            createFuncError.message,
          );
        } else {
          console.log("Temporary function created");

          // Execute the function
          const { error: execFuncError } =
            await supabase.rpc("temp_fix_auth_user");

          if (execFuncError) {
            console.error(
              "Error executing temporary function:",
              execFuncError.message,
            );
          } else {
          ...
          if (dropFuncError) {
            console.error(
              "Error dropping temporary function:",
              dropFuncError.message,
            );
          } else {
