...
import { Budget, BudgetTracking, Category } from "@/types/financial";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function BudgetOverview({
  ...
  const totalBudget = tracking.reduce(
    (sum, item) => sum + item.amount,
    0,
  );
  const totalSpent = tracking.reduce(
  ...
  const expenseTracking = tracking.filter(
    (item) => item.category?.type === "expense",
  );
  const incomeTracking = tracking.filter(
    (item) => item.category?.type === "income",
  );

  // Calculate totals by type
  const totalExpenseBudget = expenseTracking.reduce(
    (sum, item) => sum + item.amount,
    0,
  );
  const totalExpenseSpent = expenseTracking.reduce(
    (sum, item) => sum + item.spent,
    0,
  );
  const percentExpenseSpent =
    totalExpenseBudget > 0 ? (totalExpenseSpent / totalExpenseBudget) * 100 : 0;

  const totalIncomeBudget = incomeTracking.reduce(
    (sum, item) => sum + item.amount,
    0,
  );
  const totalIncomeReceived = incomeTracking.reduce(
    (sum, item) => sum + item.spent,
    0,
  );
  const percentIncomeReceived =
    totalIncomeBudget > 0 ? (totalIncomeReceived / totalIncomeBudget) * 100 : 0;
            ...
            <div className="space-y-4">
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <span>
                    {formatCurrency(totalExpenseBudget - totalExpenseSpent)}{" "}
                    remaining
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <span>
                    {formatCurrency(totalIncomeBudget - totalIncomeReceived)}{" "}
                    pending
                  </span>
                </div>
              </div>
