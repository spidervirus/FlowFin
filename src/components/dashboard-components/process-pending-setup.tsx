...
export default function ProcessPendingSetup() {
  ...
  useEffect(() => {
    const processPendingSetup = async () => {
      ...
      if (shouldProcess) {
        console.log("setupComplete parameter found, ensuring localStorage is set");
      ...
      try {
        ...
        if (!userId) {
          ...
          try {
            ...
            if (sessionUserId) {
              ...
              console.log("Got user ID from sessionStorage currentUserId:", userId);
            }
            ...
            if (!userId) {
              ...
              if (localUserId) {
                userId = localUserId;
                console.log("Got user ID from localStorage currentUserId:", userId);
              }
            }
              ...
              if (userDataStr) {
                try {
                  ...
                  if (userData?.user?.id) {
                    ...
                    console.log("Got user ID from localStorage userData:", userId);
                  }
                } catch (parseError) {
              ...
              if (supabaseAuthStr) {
                try {
                  ...
                  if (supabaseAuth?.currentSession?.user?.id) {
                    ...
                    console.log("Got user ID from localStorage supabase.auth.token:", userId);
                  }
                } catch (parseError) {
                  console.error("Error parsing supabase.auth.token from localStorage:", parseError);
                }
              }
            }
            ...
            if (!userId) {
              const sessionUserStr = sessionStorage.getItem("supabase.auth.token");
              if (sessionUserStr) {
                try {
                  ...
                  if (sessionData?.currentSession?.user?.id) {
                    ...
                    console.log("Got user ID from sessionStorage supabase.auth.token:", userId);
                  }
                } catch (e) {
        ...
        if (!userId) {
          ...
          localStorage.setItem("setupError", "Could not determine user ID. Please sign in again.");
          
          // If we have an auth error, redirect to sign-in
          if (authError) {
            // Redirect to sign-in
            router.push('/sign-in?error=Your session has expired. Please sign in again to complete setup.');
            return;
          }
          
          // Even without an auth error, we should redirect to sign-in if we can't determine the user ID
          router.push('/sign-in?error=Please sign in again to complete your account setup.');
          return;
        }
        ...
        try {
          ...
          if (settingsError && settingsError.code !== 'PGRST116') {
            console.error("Error checking for existing company settings:", settingsError);
          }
          ...
          if (existingSettings?.id) {
            ...
            try {
              ...
              console.error("Exception creating company settings:", companyError);
            }
          }
        } catch (settingsError) {
          ...
          try {
            ...
            const { error: goalError } = await supabaseClient
              ...
              .insert({
                ...
                description: setupData.goal.description,
        ...
        setError(err.message || "An error occurred while processing your setup data");
      } finally {
