import { BudgetInsight, CompanySettings } from '@/hooks/useBudgetingData';
...
export default function BudgetInsights({ insights, settings }: BudgetInsightsProps) {
      ...
      {insights.map((insight) => (
        <Card 
          ...
          className={`overflow-hidden border-l-4 ${
            insight.type === 'income' ? 'border-l-green-500' :
            insight.type === 'overspending' ? 'border-l-red-500' :
            insight.type === 'savings' ? 'border-l-blue-500' :
            'border-l-purple-500'
          }`}
        >
