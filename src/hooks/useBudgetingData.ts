...
export function useBudgetingData() {
  ...
  const fetchData = useCallback(async (forceUserId?: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "An error occurred while fetching budgeting data");
      setBudgetInsights([]);
      setMonthlyTrends({ months: [], income: [], expenses: [], savings: [], categories: [] });
      setRecommendations([]);
      toast.error(err instanceof Error ? err.message : 'Failed to fetch budgeting data');
    } finally {
      setLoading(false);
    }
  }, [userId]);
