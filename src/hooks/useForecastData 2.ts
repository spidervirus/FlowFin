...
// Raw transaction data from Supabase
...
// Raw recurring transaction data from Supabase
interface RawRecurringTransaction extends RawTransaction {
  frequency: string;
  start_date: string;
  end_date?: string;
...
export function useForecastData(
  forecastPeriod: "3months" | "6months" | "12months",
): ForecastData {
    ...
    async (forceUserId?: string) => {
      ...
      try {
        ...
        if (!settingsData || (settingsError && settingsError.code === "PGRST116")) {
          console.log("Creating default company settings for user:", effectiveUserId);
        ...
        // Use fetched or created settings
        ...
        const enrichedTransactions = (transactionsData as RawTransaction[]).map((transaction) => ({
          ...transaction,
          user_id: effectiveUserId,
        }));
        ...
        const enrichedRecurring = ((recurringData || []) as RawRecurringTransaction[]).map((transaction) => ({
          ...transaction,
          user_id: effectiveUserId,
        }));
        ...
        console.log(
          "Making forecast API request with userId:",
          effectiveUserId,
        );
