...
async function performRobustOcr(
  imagePath: string,
): Promise<{ text: string; lines: LineData[] }> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Input file not accessible: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
...
async function processReceipt(file: File): Promise<ExtractedReceiptData> {
  ...
  if (!isPdf && !isImage) {
    ...
    throw new Error(
      `Unsupported file type: ${file.type}. Only PDF and image files are supported.`,
    );
  }
  ...
  try {
    ...
    if (isPdf) {
      ...
      try {
        ...
        console.log(
          `OCR result from converted PDF: ${text.length} characters, ${lines.length} lines`,
        );
      } catch (pdfError) {
      ...
      try {
        ...
        lines = ocrResult.lines;
        console.log(
          `OCR completed: ${text.length} characters, ${lines.length} lines`,
        );
      } catch (ocrError) {
    ...
    if (text && text.trim() && text.trim().length >= 20) {
      ...
      return {
        ...
        items: [
          {
            description:
              "Failed to extract text from receipt. Please ensure the image is clear and well-lit.",
            amount: 0,
          },
        ],
      };
