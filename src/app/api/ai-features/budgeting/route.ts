...
export async function POST(request: NextRequest) {
  try {
    ...
    let effectiveUserId = session?.user?.id || userId;
    ...
    try {
      ...
      if (settingsError && settingsError.code !== "PGRST116") {
        console.error("[Budget API] Error fetching company settings:", settingsError);
      } else if (settingsData) {
        companySettings = settingsData;
        console.log("[Budget API] Found company settings for:", settingsData.company_name);
      }
    } catch (err) {
    ...
    if (transactions.length === 0) {
      try {
        const { data: transactionsData, error: transactionsError } = await adminClient
          .from("transactions")
          .select(`
            *,
        ...
        if (transactionsError) {
          console.error("[Budget API] Error fetching transactions:", transactionsError);
          return NextResponse.json({
            success: false,
    ...
    effectiveTransactions = effectiveTransactions.filter((t: any) => 
      t.user_id === effectiveUserId || !t.user_id
    );
    ...
    if (effectiveTransactions.length === 0) {
      return NextResponse.json({
        ...
        data: {
          budgetInsights: [],
          monthlyTrends: { months: [], income: [], expenses: [], savings: [], categories: [] },
          recommendations: []
        },
    ...
    console.log(`[Budget API] Analyzing ${effectiveTransactions.length} transactions`);

    // Process data to generate budget insights
    // This could be extended with actual ML/AI processing in the future
    const budgetInsights = generateBudgetInsights(effectiveTransactions, companySettings);
    const monthlyTrends = generateMonthlyTrends(effectiveTransactions, companySettings);
    const recommendations = generateRecommendations(effectiveTransactions, companySettings);
...
function generateBudgetInsights(transactions: any[], settings: CompanySettings) {
  try {
    ...
    if (currentIncome > 0) {
      ...
      insights.push({
        ...
        description: `Your income ${Math.abs(incomeChange) > 1 ? 
          (incomeChange > 0 ? 'increased' : 'decreased') + ` by ${Math.abs(incomeChange).toFixed(1)}%` : 
    ...
    if (currentExpenses > 0) {
      ...
      insights.push({
        ...
        description: `Your expenses ${Math.abs(expenseChange) > 1 ? 
          (expenseChange > 0 ? 'increased' : 'decreased') + ` by ${Math.abs(expenseChange).toFixed(1)}%` : 
          'remained stable'} compared to last month.`,
...
function generateRecommendations(transactions: any[], settings: CompanySettings) {
  try {
    const recs = [];
    ...
    if (income < expenses) {
      recs.push({
        ...
        description: 'Your expenses exceed your income. Consider reducing non-essential expenses or finding additional income sources.',
