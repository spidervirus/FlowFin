...
export async function POST(request: Request) {
  try {
    ...
    console.log("Checking if service role key is defined:", !!process.env.SUPABASE_SERVICE_ROLE_KEY);
    ...
    try {
      ...
      const { error: sqlError } = await supabaseAdmin.rpc('create_company_settings', {
        p_user_id: user.id,
      ...
      if (sqlError) {
        ...
        if (createError) {
          console.error("Error creating company settings with direct insert:", createError);
          return NextResponse.json(
