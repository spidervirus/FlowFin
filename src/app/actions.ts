...
export const signUpAction = async (formData: FormData) => {
  ...
  try {
    ...
    try {
      const { data: existsData, error: existsError } = await serviceClient.rpc(
    ...
    try {
      const { data: authDiagnostics, error: diagError } =
        await serviceClient.rpc("diagnose_auth_system");
    ...
    try {
      ...
      if (robustError) {
        console.error(
          "Error creating user with robust v2 function:",
          robustError,
        );
        userError = robustError;
      } else if (robustData && robustData.success) {
        console.log(
          "User created successfully with robust v2 function:",
          robustData,
        );
        userData = {
          user: {
        ...
        if (profileError) {
          ...
          if (backupError) {
            console.error("Error creating backup profile:", backupError);
          }
        }

        // Return success message and redirect to setup page
        return {
        ...
        robustData &&
        !robustData.success &&
        robustData.error_code === "USER_EXISTS"
      ) {
        console.log(
          "User already exists according to robust v2 function:",
          robustData,
        );
        // Redirect to sign-in page with a message that the user already exists
        return {
      ...
      console.error(
        "Exception creating user with robust v2 function:",
        robustError,
      );
    ...
    try {
      ...
      if (robustError) {
        ...
        console.log(
          "User already exists according to robust function:",
          robustData,
        );
        // Redirect to sign-in page with a message that the user already exists
        return {
      ...
      console.error(
        "Exception creating user with robust function:",
        robustError,
      );
      // Continue to next method
    }
    ...
    if (!userData?.user) {
      try {
        ...
        console.error(
          "Error in alternative sign-up approach:",
          alternativeError,
        );
    ...
    if (!userData?.user) {
      console.log(
        "Standard auth methods failed, trying manual registration bypass...",
      );

      try {
        // Use our manual registration function
        const { data: manualData, error: manualError } =
          await serviceClient.rpc("manual_register_user", {
            p_email: email,
            p_password: password,
        ...
        if (manualData && manualData.success) {
          console.log(
            "User created successfully via manual bypass:",
            manualData,
          );
    ...
    console.error("Unexpected error in signUpAction:", typedError);
    ...
    if (error) {
      ...
      try {
        const { data: existsData, error: existsError } =
          await serviceRoleClient.rpc("check_user_exists_by_email", {
            p_email: email,
          });
        ...
        if (existsError) {
          ...
          try {
            const { data: bypassData, error: bypassError } =
              await serviceRoleClient.rpc("bypass_auth_for_user", {
                p_email: email,
                p_password: password,
              });
          ...
          try {
            ...
            if (userId) {
              ...
              if (createError) {
                console.error(
                  "Error creating user with specific ID:",
                  createError.message,
                );
              } else {
...
export const forgotPasswordAction = async (formData: FormData) => {
  ...
  try {
    ...
    redirect(
      "/forgot-password?error=An error occurred while sending the reset link",
    );
  }
};
