...
export default function GoalPage({ params }: GoalPageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch contributions
        const contributionsResponse = await fetch(`/api/goals/contributions?goal_id=${params.id}`);
  ...
  const handleDeleteContribution = async (contributionId: string) => {
    try {
      const response = await fetch(`/api/goals/contributions?id=${contributionId}`, {
        method: 'DELETE',
      });
                    ...
                    <div className="space-y-4">
                      ...
                      {contributions.length === 0 ? (
                        <div className="text-center py-8">
                          ...
                          <Link href={`/dashboard/goals/${params.id}/contribute`}>
                            <Button>
                              <Plus className="mr-2 h-4 w-4" /> Add Your First Contribution
                            </Button>
                          </Link>
                        </div>
                      ) : (
                              ...
                              <div>
                                ...
                                {contribution.transaction && (
                                  <div className="text-xs text-muted-foreground mt-1">
                                    Linked to transaction: {contribution.transaction.description}
                                  </div>
                                )}
                              </div>
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      Are you sure you want to delete this contribution of {formatCurrency(contribution.amount)}?
                                      This action cannot be undone.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    {!goal.is_completed && (
                      <div className="pt-4">
                        <Link href={`/dashboard/goals/${params.id}/contribute`} className="w-full">
