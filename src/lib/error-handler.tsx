...
const errorCodeMessages: Record<string, string> = {
  ...
  "auth/requires-recent-login":
    "This operation requires a more recent login. Please sign in again.",
  ...
  "auth/maximum-tenant-count-exceeded":
    "The maximum number of tenants has been exceeded.",
  ...
  "auth/invalid-credential-or-provider-id":
    "The provided credential or provider ID is invalid.",
  ...
  "auth/invalid-oauth-response-type":
    "The provided OAuth response type is invalid.",
  ...
  "auth/already-initialized":
    "initializeAuth() has already been called with different options. To avoid this error, call initializeAuth() with the same options as when it was originally called.",
  ...
  "database/invalid-argument":
    "Invalid data was provided to the database operation.",
  ...
  "database/resource-exhausted":
    "The database has reached its resource limits. Please try again later.",
  ...
  "validation-error":
    "The data you provided is invalid. Please check your inputs and try again.",
  ...
  "gateway-timeout":
    "The server timed out waiting for another server. Please try again later.",
};
...
export function handleAuthorizationError(error: any): AppError {
...
export function handleError(error: any): AppError {
  if (!error) {
    return handleUnknownError(new Error("No error provided"));
  }
  ...
  // Handle database errors
  if (error.name === "DatabaseError" || error.type === "database") {
    return handleDatabaseError(error);
  }

  // Handle authorization errors
  if (error.name === "AuthorizationError" || error.type === "authorization") {
    return handleAuthorizationError(error);
  }

  // Handle server errors
  if (error.name === "ServerError" || error.type === "server") {
    return handleServerError(error);
  }
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    showErrorToast(this.state.error || handleError(error));
  }
