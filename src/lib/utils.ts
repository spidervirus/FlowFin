...
export const formatCurrency = (
  ...
  return new Intl.NumberFormat(config.locale, {
    ...
    minimumFractionDigits:
      options?.minimumFractionDigits ?? config.minimumFractionDigits ?? 0,
...
export const getUserCurrency = (): CurrencyCode => {
  ...
  try {
    ...
    if (setupDataStr) {
      ...
      if (
        setupData.currency &&
        CURRENCY_CONFIG[setupData.currency as CurrencyCode]
      ) {
      ...
      if (
        setupData.companySettings?.default_currency &&
        CURRENCY_CONFIG[
          setupData.companySettings.default_currency as CurrencyCode
        ]
      ) {
    ...
    if (userPrefsStr) {
      ...
      if (
        userPrefs.currency &&
        CURRENCY_CONFIG[userPrefs.currency as CurrencyCode]
      ) {
