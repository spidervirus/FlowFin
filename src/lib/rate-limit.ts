...
export class RateLimit {
  ...
  async check(request: NextRequest): Promise<NextResponse | null> {
    try {
      ...
      headers.set('X-RateLimit-Remaining', Math.max(0, this.config.maxRequests - requestCount).toString());
      headers.set('X-RateLimit-Reset', (Date.now() + this.config.windowMs).toString());
      ...
      logger.error('Rate limit error', error instanceof Error ? error : new Error('Unknown error'));
...
export const strictRateLimit = {
  check: async (request: NextRequest) => {
    try {
      ...
      logger.error('Error in rate limit check', error instanceof Error ? error : new Error(String(error)));
